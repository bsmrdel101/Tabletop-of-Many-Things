...
export const spellDetailsHtml = (spell: Spell): string => {
  return `
    <div class="modal-stats__spell-details">
      <h4>${spell.name}</h4>
      <p class="modal-stats__spell-details--subtitle">${spell.level === 0 ? 'cantrip' : `${spell.level}st level`} ${spell.school} ${spell.ritual ? '(ritual)' : ''}</p>
      <p><span class="bold">Casting Time</span> ${spell.castingTime}</p>
      <p><span class="bold">Range</span> ${spell.range}</p>
      <p><span class="bold">Components</span> ${spell.components.map((component: string) => capitalize(component)).join(', ')} ${spell.material ? `(${spell.material})` : ''}</p>
